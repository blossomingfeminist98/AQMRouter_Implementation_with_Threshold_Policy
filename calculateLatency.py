...
#-----------------------------------------------------------------
#               Calculating Queueing Delay 
#        Queueing Delay = QueueSize / Router Capacity 
#-----------------------------------------------------------------
def calculate_QueueingDelay(f1, QueueType, RTT, Metric):
    file = pd.read_csv(f1, delimiter='\t', header=None)
    filename = "./FinalResults/TCPLinuxReno/" + QueueType + "/" + QueueType +  RTT + "RTT" + Metric + ".txt" 
    QD = open(filename, "a")
    ...
    for i in range(200,250):
        QD.write(str(i) + "\t")
        queueing_delay[0] = R1[i] / 100
        QD.write(str(queueing_delay[0]) + "\t")
        queueing_delay[1] = round(R2[i] / 40, 3)
        QD.write(str(queueing_delay[1]) + "\t")
        queueing_delay[2] = round(R3[i] / 40, 3)
        QD.write(str(queueing_delay[2]) + "\t")
        queueing_delay[3] = round(R4[i] / 60, 3)
        QD.write(str(queueing_delay[3]) + "\t")
        queueing_delay[4] = round(R5[i] / 60, 3)
        QD.write(str(queueing_delay[4]) + "\t")
        queueing_delay[5] = round(R6[i] / 60, 3)
...
def calculate_latency(f2, QueueType, str_RTT, RTT, Metric):
    ...
    filename = "./FinalResults/TCPLinuxReno/" + QueueType + "/" + QueueType + str_RTT + "RTT" + Metric + ".txt" 
    QD = open(filename, "a")
...
def find_mean_SDV(f2, QueueType, RTT, Metric):
    file = pd.read_csv(f2, delimiter='\t', header=None)
    filename = "./FinalResults/TCPLinuxReno/Statistics/" + QueueType + "_Mean_" + Metric + ".txt" 
    M = open(filename, "a")
    filename = "./FinalResults/TCPLinuxReno/Statistics/" + QueueType + "_SD_" + Metric + ".txt" 
    SD = open(filename, "a")
...
for i in queue_size:
    for j in round_trip_time:
        if(i=="Threshold100" or i=="Threshold15"):
            f1 = "./FinalResults/TCPLinuxReno/" + i + "/" + i + str(j) + "msRTTQS.txt"
        else: 
            f1 = "./FinalResults/TCPLinuxReno/" + i + "/" + i + str(j) + "RTTQS.txt"
        f2 = "./FinalResults/TCPLinuxReno/" + i + "/" + i + str(j) + "RTTQD.txt"; 
        calculate_QueueingDelay(f1, str(i), str(j), "QD")
        calculate_latency(f2, str(i), str(j), j, "Lat")
        # print(f1, f2, f3, f4)
for i in queue_size:
    for j in round_trip_time:
        f3 = "./FinalResults/TCPLinuxReno/" + i + "/" + i + str(j) + "RTTLat.txt"; 
